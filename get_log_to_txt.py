...
def train(cfg):
    ...
    while epoch_it < (scheduling_start + scheduling_epoch):
        ...
        loss_list.append("{} {} {}\n".format(pc_loss_epoch,rgb_s_loss_epoch,num/index))
        ...
        if (eval_img_every>0 and (epoch_it % eval_img_every) == 0):    
            ...
            psnr_list.append("{} {}\n".format(epoch_it,psnr))
            with open('psnr.txt','w') as f:
                f.writelines(psnr_list)
...
def show():
    ...
    x_data= []
    x_data.append(epoch_it)
    y__data.append(loss)
    plt.plot(x_data, y__data, marker='*', ms=10,label='the loss of training ')
    plt.legend()  # 让图例生效
